...
public class ViewTopTenBorrowed {

    private static String FORMAT = "%-5s|%-20s|%-12s|%-50s|%-50s|%-12s|%-12s|%-2s";

    public ViewTopTenBorrowed() {

    }

    public static void viewTopTenBorrowed(String username) throws SQLException {
        ...
        try {
            ...
            String query = "select row_number() over (order by total_days_borrowed/(total_days::decimal) DESC) as ranking,\n" +
                    "       round((total_days_borrowed/(total_days::decimal)) * 100, 2) as percentage_borrowed,\n" +
                    ...
                    "            WHEN date_returned IS NOT NULL THEN date_returned END)::DATE - request.date_responded::DATE) as total_days_borrowed\n" +
                    "    from request\n" +
                    "    join tool t on t.barcode = request.tool_barcode\n" +
                    "    where username = (?) AND status in ('Returned', 'Accepted')\n" +
                    "    group by tool_barcode, total_days) as inner_table\n" +
                    "join tool t2 on inner_table.tool_barcode = t2.barcode\n" +
                    "order by percentage_borrowed desc\n" +
                    "limit 10";
            ...
            if (!result.next()) {
                ...
                do {
                    ...
                    String percentage = result.getString("percentage_borrowed");
